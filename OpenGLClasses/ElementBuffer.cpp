...
ElementBuffer::ElementBuffer(unsigned int size, void *data, int drawOption) {
	GL_ERROR_WRAPPER(glGenBuffers(1, &(this->m_bufferId)));
	this->bind();
...
ElementBuffer::~ElementBuffer() {
	GL_ERROR_WRAPPER(glDeleteBuffers(1, &(this->m_bufferId)));
}

void ElementBuffer::bind() {
	GL_ERROR_WRAPPER(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, this->m_bufferId));
}
